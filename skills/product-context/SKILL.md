...
When the user says "look up idea X in Revelica," you get back the hypothesis, all supporting and contradicting evidence, the activity log, connections to opportunities and segments, and the current status. That's hours of conversation compressed into one `query` call.
...
```
query(query="*", artifact_type="idea", depth="summary")
```
...
- **`query(query, artifact_type?, depth?)`** — search. Returns the schema of the type plus matching resources. Default `depth="summary"`; pass `depth="full"` when you need whole content.
- **`create(artifact_type, name, content, parent_id?, project_id?)`** — create a new artifact or entity. Pass `parent_id` to create a new version of an existing artifact instead of a fresh one.
- **`update(artifact_id, updates, artifact_type?)`** — patch specific fields without a full rewrite.

Plus `load_skill(skill_name)` to load task-specific skills (see below), and `list_skills()` to see what's available.

Every `artifact_type` has a schema — call `query(query="*", artifact_type="<type>")` to see it before creating.
...
```
create(artifact_type="document", name="...", content={
...
Call `list_skills()` to see all skills available to MCP clients in your workspace. Common ones:
...
```
query(query="*", artifact_type="customer-segment", depth="full")   # target customer, JTBD, use cases
query(query="*", artifact_type="value-proposition", depth="full")  # competitive positioning, differentiation
query(query="*", artifact_type="company", depth="full")            # competitors and own company
query(query="*", artifact_type="idea", depth="full")               # the hypotheses being bet on
query(query="*", artifact_type="opportunity", depth="full")        # customer pain points and unmet needs
query(query="*", artifact_type="hypothesis", depth="full")         # active bets with idea refs + impacts
```
...
```
query(query="*", artifact_type="story-map", depth="full")
```
...
```
create(artifact_type="story-map", name="Onboarding flow", content={...})
```

Call `query(query="*", artifact_type="story-map")` first to get the schema.
...
```
create(artifact_type="idea", name="<short name>", content={
  "name": "<short name>",
...
That's hypothesis-shaped, not idea-shaped. Hypotheses are project-scoped and structured around `idea_refs` + `impacts` + `assumption_ids`. Call `query(query="*", artifact_type="hypothesis")` once to see the full schema before creating. Typical flow:

1. If the claim references a new idea, `create(artifact_type="idea", ...)` first to get an idea_id
2. `create(artifact_type="hypothesis", ...)` referencing that idea_id in `idea_refs`, the relevant project's `project_id`, the KR being claimed in `impacts`, and any assumptions the claim depends on
...
```
query(query="<topic>", artifact_type="evidence-source", depth="full")
query(query="<topic>", artifact_type="evidence-finding", depth="full")
```
...
```
create(artifact_type="evidence-source", name="<descriptive label>", content={
  "source_type": "url",
...
- Things already in the workspace (`query` first to check for duplicates)
